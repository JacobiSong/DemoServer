        ...
        switch (type) {
            ...
            case LOGIN: {
                ...
                if (DaoUtil.loginCheck(username, password)) { // 密码正确
                    try { // 生成token
                        final String token = MessageDigest.getInstance("md5").digest(UniqId.getInstance().getUniqID().getBytes()).toString();
